...
> ### 这是我做个爬虫时遇到问题，当时抓出来的数据有一列数据是这样的：张三/[2017-01-21],而且输出了DataFrame。需要把这列拆分为姓名和日期两个数据列，强迫症的我又不想回头去处理原始文本数据，就想在DataFrame做手脚，我看过的书和教程都没提到这个，网上搜了半天也没提到什么直接的方法能做到，搞了半天终于弄出来了，所以赶紧写下来不然回头又忘记了
...
### 重要结论：分别看series和np.array的第二例子！！列表list作为输入数据，series和np.array两者得到结果完全不同
